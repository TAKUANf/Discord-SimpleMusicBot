...
import type { LavalinkStrategy } from "./lavalink";
...
// Strategy-level timeout: 15s for library-based, 30s for binary-based (yt-dlp, youtube-dl, nightly)
const STRATEGY_TIMEOUT_MS = 15_000;
const BINARY_STRATEGY_TIMEOUT_MS = 30_000;

// Binary-based strategy indices (yt-dlp = 7, youtube-dl = 6, nightly = 8)
// Lavalink (9) uses REST API with its own timeout, treat as non-binary
...
const strategyImporters: StrategyImporter[] = [
  ...
  { enable: !!config.lavalink?.enabled, isFallback: false, importer: () => require("./lavalink") },
] as const;
